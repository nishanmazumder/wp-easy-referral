	...
	private function handle_front_register() {
		...
		if ( 'referred' === $form_context || 'dashboard_referral' === $form_context ) {
			...
			wp_safe_redirect(
				add_query_arg(
					array(
						'ref'          => rawurlencode( $referred_by_code ),
						'wperf_thanks' => '1',
					),
					$this->get_shared_registration_page_url()
				)
			);
			exit;
		}
